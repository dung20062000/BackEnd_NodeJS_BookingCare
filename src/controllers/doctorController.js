...
let handleGetProFileDoctorById = async (req, res) => {
    try{
        let info = await doctorService.handleGetGetProFileDoctorByIdService(req.query.doctorId)
        return res.status(200).json(info) 
    }catch(err){
        console.log(err)
        return res.status(200).json({
...
let handleGetListPatientForDoctor = async(req, res) => {
    try{
        let info = await doctorService.handleGetListPatientForDoctorService(req.query.doctorId, req.query.date)
        return res.status(200).json(info) 
    }catch(err){
        console.log(err)
        return res.status(200).json({
...
let sendRemedy = async(req, res) => {
    try{
        let info = await doctorService.handleSendRemedyService(req.body)
        return res.status(200).json(info) 
    }catch(err){
        console.log(err)
        return res.status(200).json({
